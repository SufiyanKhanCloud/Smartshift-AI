...
function buildReportHTML() {
  // Settings live in the Supabase-backed form fields (loadSettings populates
  // them); read them straight from there rather than a stale localStorage key.
  const bizName    = document.getElementById('setting-business-name')?.value || 'SmartShiftAI';
...
<div class="report-container">
  ...
  <section style="background:#f7fafc;text-align:center;padding:20px 40px">
    <p style="font-size:0.78rem;color:#a0aec0;margin:0">Generated by <strong>SmartShiftAI</strong> - AI Workforce Planning System - Final Year Project<br>
    Powered by Flask · scikit-learn · Python 3</p>
    <div class="no-print" style="margin-top:12px;font-size:0.82rem;color:#718096">
      Press <strong>Ctrl&nbsp;+&nbsp;P</strong> (or <strong>&#8984;&nbsp;+&nbsp;P</strong>) to save this report as a PDF,
      or use the <strong>Print / Save as PDF</strong> button on the Report page.
    </div>
  </section>
</div>
</body></html>`;
...
/* ---- Open the report in a new tab for viewing ----
   Uses a Blob URL rather than an about:blank popup so the URL is a real link.
   The tab inherits the app's strict CSP, so it can't run its own JS — hence the
   "Print / Save as PDF" action lives on the main Report page (printReport). ---- */
...
/* ---- Print / Save as PDF ----
   Renders the report into a hidden same-origin iframe and calls print() on it
   from THIS window's (CSP-allowed) script. Calling print() on a same-origin
   iframe is a direct API call, not blocked by the Content-Security-Policy, so
   this works where an in-popup button cannot. ---- */
function printReport() {
  ...
  showToast('Opening print dialog - choose "Save as PDF" as the destination.', 'success');
}
